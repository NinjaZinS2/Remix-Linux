...
static void DrawLibBar(Color ab,Color white,Color gray){
    ...
    if(g_view==2){
        ...
        if(const Playlist* op=OpenPlaylistPtr()){ std::wstring m=op->mode.empty()?(g_cfg.onlineMode==L"download"?L"ONLINE: BAIXAR":L"ONLINE: STREAM"):(op->mode==L"download"?L"ONLINE: BAIXAR ●":L"ONLINE: STREAM ●"); DrawPill(R_plMode,m,!op->mode.empty(),S(10)); }
    } else if(g_pickMode){
