...
//  - yt-dlp: YouTube, YouTube Music, SoundCloud, Bandcamp e centenas de sites.
//    Desde a versao 2025.11 o YouTube exige um runtime JavaScript (deno, node ou bun).
//  - spotdl: le musicas/albuns/playlists do Spotify (so os metadados).
...
//  dos arquivos): titulo + artista sao procurados no YouTube Music, como o spotdl faz.
#include "platform.h"
#include "config.h"
#include "playlist.h"
...
inline std::vector<std::wstring> OToolDirs() {
    ...
    if (h) { std::wstring hw = Utf8ToWide(h); d.push_back(hw + L"/.local/bin"); d.push_back(hw + L"/.deno/bin"); d.push_back(hw + L"/bin"); }
    for (const wchar_t* x : { L"/usr/local/bin", L"/usr/bin", L"/bin", L"/snap/bin" }) d.push_back(x);
    d.push_back(Config::Join(Config::BaseDir(), L"tools")); d.push_back(Config::ExeDir());
#endif
    return d;
}
inline std::wstring OFindTool(const std::wstring& name) {
    for (auto& dir : OToolDirs()) {
#ifdef _WIN32
        std::wstring p = Config::Join(dir, name + L".exe");
        DWORD a = GetFileAttributesW(p.c_str());
        if (a != INVALID_FILE_ATTRIBUTES && !(a & FILE_ATTRIBUTE_DIRECTORY)) return p;
#else
        std::wstring p = Config::Join(dir, name);
        std::string u = WideToUtf8(p);
        struct stat sb;
        if (access(u.c_str(), X_OK) == 0 && stat(u.c_str(), &sb) == 0 && S_ISREG(sb.st_mode)) return p;
#endif
    }
    return L"";
}
inline void ProbeOnlineToolsImpl() {
    ...
    if (yt.empty() && !sp.empty()) {   // sem yt-dlp solto: usa o que veio junto com o spotdl (python -m yt_dlp)
        ...
        if (first.rfind("#!", 0) == 0) {
            std::string interp = first.substr(2); while (!interp.empty() && interp[0] == ' ') interp.erase(0, 1);
            size_t sp1 = interp.find(' '); if (sp1 != std::string::npos) interp = interp.substr(0, sp1);
            CapResult r = RunCapture({ Utf8ToWide(interp), L"-c", L"import yt_dlp" }, 20000);
            if (r.code == 0) py = Utf8ToWide(interp);
        }
    }
#endif
    ...
    std::wstring vYt = !yt.empty() ? ver({ yt, L"--version" }) : (!py.empty() ? ver({ py, L"-m", L"yt_dlp", L"--version" }) : L"");
...
inline void EnsureTools() { if (!OT().probed.load()) ProbeOnlineTools(); }
inline bool YtdlpOk() { std::lock_guard<std::mutex> lk(OT().m); return !OT().ytdlp.empty() || !OT().ytPython.empty(); }
inline bool FfmpegOk() { std::lock_guard<std::mutex> lk(OT().m); return !OT().ffmpeg.empty(); }
inline std::wstring FfmpegTool() { std::lock_guard<std::mutex> lk(OT().m); return OT().ffmpeg; }
inline std::vector<std::wstring> YtdlpArgs() {
    std::lock_guard<std::mutex> lk(OT().m);
    std::vector<std::wstring> a;
    if (!OT().ytdlp.empty()) a.push_back(OT().ytdlp); else { a.push_back(OT().ytPython); a.push_back(L"-m"); a.push_back(L"yt_dlp"); }
    for (const wchar_t* x : { L"--ignore-config", L"--no-warnings", L"--encoding", L"utf-8", L"--socket-timeout", L"20" }) a.push_back(x);
    if (!OT().jsPath.empty() && OT().vYt >= L"2025.11") { a.push_back(L"--js-runtimes"); a.push_back(OT().jsName + L":" + OT().jsPath); }
    if (!OT().ffmpeg.empty()) { a.push_back(L"--ffmpeg-location"); a.push_back(OT().ffmpeg); }
    return a;
}
...
inline bool OnlineSearch(const std::wstring& q, int where, std::vector<OTrack>& out, std::wstring& err, const std::atomic<bool>* cancel, int limit = 20,
    ...
    EnsureTools();
    if (!YtdlpOk()) { err = L"Instale o yt-dlp para buscar online (veja Configurações > ONLINE)."; return false; }
    auto a = YtdlpArgs(); a.push_back(L"-j"); a.push_back(L"--flat-playlist");   // -j: uma linha JSON por resultado (a tela mostra conforme chegam)
    ...
    CapResult r = RunCapture(a, 90000, cancel, [&](const std::string& ln) {
...
inline void ResolveSpotifyDl(const std::wstring& url, OResolved& r, const std::atomic<bool>* cancel) {   // reserva: spotdl (lento: a API do Spotify ficou restrita)
    EnsureTools();
    std::wstring sp; { std::lock_guard<std::mutex> lk(OT().m); sp = OT().spotdl; }
    if (sp.empty()) { r.err = L"Links do Spotify precisam do spotdl instalado (pip install spotdl)."; return; }
    std::error_code ec;
    std::wstring dir = Config::Join(Config::CacheDir(), L"online"); std::filesystem::create_directories(std::filesystem::path(dir), ec);
    std::wstring tmp = Config::Join(dir, L"resolve-" + std::to_wstring((unsigned long long)GetTickCount64()) + L".spotdl");
    CapResult cr = RunCapture({ sp, L"save", url, L"--save-file", tmp, L"--log-level", L"ERROR" }, 600000, cancel);
    std::string data;
    if (!ReadFileUtf8(tmp, data)) { r.err = cr.canceled ? L"Cancelado." : L"O spotdl não conseguiu ler esse link do Spotify."; return; }
    std::filesystem::remove(std::filesystem::path(tmp), ec);
    JVal v;
    if (!OParse(data, v) || v.t != JVal::ARR) { r.err = L"O spotdl devolveu algo inesperado."; return; }
    for (auto& x : v.a) {
        if (x.t != JVal::OBJ) continue;
        OTrack t; t.src = OS_SPOTIFY; t.title = Utf8ToWide(JS(x, "name")); t.url = Utf8ToWide(JS(x, "url"));
        std::string ar = JS(x, "artist");
        if (const JVal* as = x.get("artists")) if (as->t == JVal::ARR) { std::string all; for (auto& a : as->a) if (a.t == JVal::STR) { if (!all.empty()) all += ", "; all += a.s; } if (!all.empty()) ar = all; }
        t.artist = Utf8ToWide(ar); t.album = Utf8ToWide(JS(x, "album_name")); t.dur = (int)x.num("duration", 0); t.thumb = Utf8ToWide(JS(x, "cover_url"));
        if (r.name.empty()) r.name = Utf8ToWide(JS(x, "list_name"));
        if (!t.title.empty()) r.items.push_back(t);
    }
    if (r.name.empty() && !r.items.empty()) r.name = r.items.size() == 1 ? r.items[0].title : r.items[0].album;
    if (r.items.empty()) r.err = L"Nenhuma música nesse link do Spotify.";
}
