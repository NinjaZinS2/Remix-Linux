...
inline int AddFiles(const std::vector<std::wstring>& files, std::wstring& err) {
    ...
    for (auto& f : files) {
        ...
        if (!Player::ProbeNative(dst)) {   // m4a/opus/wma...: vira WAV
            ...
            CapResult r = FfmpegOk() ? RunCapture({ FfmpegTool(), L"-nostdin", L"-loglevel", L"error", L"-y", L"-i", dst, L"-vn", L"-ac", L"2", L"-ar", L"48000", wav }, 120000) : CapResult();
            fs::remove(fs::path(dst), ec);
            if (r.code != 0 || !fs::exists(fs::path(wav), ec)) { err = (FfmpegOk() ? L"Não consegui ler: " : L"Formato não suportado (instale o ffmpeg): ") + src.filename().wstring(); fs::remove(fs::path(wav), ec); continue; }
            dst = wav;
        }
