...
// github.com/facebookresearch/demucs, MIT; modelo htdemucs). Opcional: o instalador de dependencias
...
//  - Musica online: baixa o audio primeiro (reaproveitando a extracao do yt-dlp em cache).
...
inline void RunJob(Job& j, const std::function<void(const std::wstring&, int)>& notify) {
    ...
    if (tool.empty()) { fail(L"O separador de stems (Demucs) não está instalado: rode o instalador de dependências."); return; }
    EnsureTools();
    if (!FfmpegOk()) { fail(L"Precisa do ffmpeg para separar os stems."); return; }
    ...
    if (j.online) {   // baixa o audio (so o audio, formato original) para separar
        ...
        ec.clear();
        if (have.empty()) {
            if (!YtdlpOk()) { fail(L"Precisa do yt-dlp para separar música online."); return; }
            ...
            for (int tent = 0; tent < 2 && have.empty() && !j.cancel; ++tent) {
                ...
                RunCapture(a, 600000, &j.cancel, nullptr, 64 * 1024);
        ...
        if (have.empty()) { fail(L"Não consegui baixar o áudio dessa música para separar."); return; }
        input = have;
    }
    j.state = S_SEPARATING; j.pct = 0; notify(j.key, j.state);
    std::vector<std::wstring> args = { PythonIn(tool), RunnerPath(), input, dir, FfmpegTool(), Config::Join(tool, L"torch") };
