...
static void BuildDcPanel(int w,int h){
    ...
    p.Pill(cell(3,5),v.installing?L"BAIXANDO...":(v.depsOk?L"ATUALIZAR BIBLIOTECAS":L"BIBLIOTECAS DO BOT (npm)"),!v.depsOk&&v.nodeOk&&!v.installing,Z_DC_INSTALL);
    ...
    y+=SI(20);
    std::wstring l2=v.probing&&!v.nodeOk?L"Procurando o Node.js...":(v.nodeOk?L"Node.js "+v.nodeVer+(v.depsOk?L"  ·  bibliotecas prontas":L"  ·  falta o discord.js: o botão roda o SEU npm (npm i discord.js @discordjs/voice) na pasta do Remix"):(v.nodeVer.empty()?L"Node.js 22.12+ não encontrado: instale o Node.js pela sua distro.":v.nodeVer));
...
static void DcClick(int z){
    ...
    if(z==Z_DC_INSTALL){ if(v.installing) return; if(!v.nodeOk){ SetStatus(L"Precisa do Node.js 22.12+: instale o Node.js pela sua distro.",4500); return; } SetStatus(L"Rodando o seu npm para trazer o discord.js...",3500); dc::InstallAsync(); return; }
