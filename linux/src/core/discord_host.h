...
//  - TODA a logica fica aqui, com os mecanismos do app: fila por servidor, votacao, permissoes, busca (CLI de midia),
...
// stdin, nunca para log ou tela. Links de quem usa o bot so de fontes conhecidas (o CLI de midia nao vira um leitor de
...
inline std::wstring FindNode(std::wstring& ver, std::wstring& why) {
    ...
    { std::wstring t = fonte::AcharNoSistema(L"node"); if (!t.empty()) cand.push_back(t); }
#else
    ...
    std::wstring sys = fonte::AcharNoSistema(L"node"); if (!sys.empty()) cand.push_back(sys);
#endif
    for (auto& c : cand) {
...
#ifndef _WIN32
    std::wstring npm = fonte::AcharNoSistema(L"npm"); if (!npm.empty()) return { npm };
#endif
    return {};
}
...
inline void InstallAsync() {
    ...
    { std::lock_guard<std::mutex> lk(s.m); s.installMsg = L"Trazendo discord.js e @discordjs/voice com o seu npm..."; }
    Changed();
    std::thread([] {
        State& st = St(); std::wstring msg;
        std::wstring ver, why; std::wstring node = FindNode(ver, why);
        if (node.empty()) msg = why.empty() ? L"Precisa do Node.js 22.12+: instale o Node.js pela sua distro." : why;
        else {
...
inline void PlayCurL(Guild& g, int offset, bool newMessage) {
    ...
    PresenceL();
    if (newMessage) {   // adianta o link das proximas online (a CLI leva ~3 s): a troca de faixa fica rapida
...
inline void OnBotL(const JVal& v, const std::string& t) {
    ...
    if (t == "state") {
        ...
        if (failed) {
            if (++g.fails >= 5) { Announce(g, L"⚠️ Várias músicas seguidas não tocaram: parei. Veja se a fonte externa e o ffmpeg do PC do dono estão em dia."); g.fails = 0; g.q.clear(); StopAudioL(g); DeleteNpL(g); PresenceL(); return; }
...
inline void ServeAudio(hsock_t c, unsigned rg) {
    ...
    if (!fonte::FfmpegOk() || (online && !fonte::Configurada())) { Send404(c); return; }
    ...
    if (pipeMode) {
        ...
        if (!fonte::Abrir(src, ya, true, false, false)) { dec.Kill(); dec.Wait(); Send404(c); return; }
...
#endif
    if (node.empty()) { err = why.empty() ? L"Precisa do Node.js 22.12+: instale o Node.js pela sua distro." : why; return false; }
    if (override_.empty() && !DepsOk()) { err = L"Faltam as bibliotecas do bot: toque em BIBLIOTECAS DO BOT (npm)."; return false; }
    if (override_.empty() && !WriteBotScript(err)) return false;
    botScript = override_.empty() ? Config::Join(BotDir(), L"bot.mjs") : override_;
    if (!fonte::FfmpegOk()) fonte::Garantir();
