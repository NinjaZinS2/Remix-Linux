...
// Cada canal tem o seu a fonte externa e o ffmpeg e o seu buffer. Canal da fila guarda so o comeco
...
inline void StreamThread(std::shared_ptr<StreamJob> j) {
    ...
    if (!fonte::Configurada() || !fonte::FfmpegOk()) { fail(fonte::MsgFalta()); return; }
    std::wstring err;
    j->phase = 1;
    OTrack t = j->t;
    ...
    { std::lock_guard<std::mutex> lk(j->im); j->via = pipeMode ? L"fonte externa | ffmpeg (pipe) -> PCM na memória" : L"fonte externa (URL do áudio) -> ffmpeg -> PCM na memória"; }
    ...
    for (int attempt = 0; !j->stop; ++attempt) {
        ...
        std::thread pump;
        if (pipeMode) {
            auto ya = fonte::CmdTocar(t.play);
            if (!fonte::Abrir(src, ya, true, false, false)) { dec.Kill(); dec.Wait(); fail(fonte::MsgFalta()); return; }
        ...
        if (!gotAny && !pipeMode && attempt < 2) {   // a URL direta nao abriu: tenta pelo pipe do CLI de midia
            ForgetMediaInfo(t.play);
            pipeMode = true;
            { std::lock_guard<std::mutex> lk(st->m); st->canRestart = false; st->pcm.clear(); st->baseFrame = 0; j->endFrame = 0; }
            { std::lock_guard<std::mutex> lk(j->im); j->via = L"fonte externa | ffmpeg (pipe) -> PCM na memória"; }
            startFrame = 0;
            continue;
        }
...
inline int MediaDurSec(const std::wstring& file) {
    if (!fonte::FfmpegOk()) return 0;
    CapResult r = RunCapture({ fonte::Ffmpeg(), L"-nostdin", L"-hide_banner", L"-i", file }, 60000);
...
inline void RunDownload(DlJob& j) {
    ...
    if (!fonte::Configurada() || !fonte::FfmpegOk()) { finish(3, fonte::MsgFalta(3)); return; }
    ...
    // O CLI de midia baixa a partir desse JSON (--load-info-json) sem extrair de novo; se falhar, faz do jeito normal.
    ...
    auto runYt = [&](bool fromInfo) {
        ...
        return fonte::Rodar(a, 0, &j.cancel, [&](const std::string& ln) { size_t p = ln.find("REMIXPCT"); if (p != std::string::npos) { float v = (float)atof(ln.c_str() + p + 8); if (v >= 0 && v <= 100) j.pct = v * 0.9f; } });
    };
    auto findProduced = [&]() {
        std::wstring f;
    ...
    // Quanto a musica deve ter. Se o arquivo baixado vier bem menor, o CLI de midia trouxe so um pedaco
    ...
    if (!t.title.empty()) {   // titulo/artista certos (links do Spotify/Deezer/Apple ou da busca)
        ...
        std::vector<std::wstring> fa = { fonte::Ffmpeg(), L"-nostdin", L"-loglevel", L"error", L"-y", L"-i", produced, L"-map", L"0", L"-c", L"copy", L"-metadata", L"title=" + t.title };
...
struct OnlineUI {
    ...
    RECT box{}, qbox{}, btnSearch{}, btnClose{}, src[3]{}, tab[3]{}, btnAddAll{}, btnCfg{};   // btnCfg: aparece quando falta a CLI
...
inline void OnlineSearchAsync() {
    ...
    std::thread([q, where, gen, link] {
        ...
        else OnlineSearch(q, where, out, err, nullptr, 20, [&](const OTrack& t) {   // resultados aparecem conforme o CLI de midia acha
