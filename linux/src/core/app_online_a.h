...
// procura a fonte externa e o ffmpeg em segundo plano (so quando o usuario chega perto do online)
static void EnsureToolsAsync(){
    if(fonte::JaConferiu()||fonte::Conferindo()) return;
    std::thread([]{ RemixSafe("procurar a fonte externa e o ffmpeg",[]{ fonte::Conferir(); }); AppPost(EV_TOOLS_READY); }).detach();
}
...
static void QueueDownloadsInto(const std::vector<OTrack>& items,const std::wstring& plName,const std::wstring& base){
    ...
    SetStatus(n?std::to_wstring(n)+(n==1?L" na fila de download  →  ":L" músicas na fila de download  →  ")+dst:std::wstring(L"Nada para salvar."),4500);
}
static void StartDownloads(std::vector<OTrack> items,const std::wstring& plName){
    ...
    if(items.empty()){ SetStatus(L"Nada para salvar.",3000); return; }
    if(g_cfg.askDlFolder){ g_pendingDl={items,plName}; SetStatus(L"Escolha a pasta onde salvar…",3000); PlatformPickFolderFor(EV_PICK_DLONCE,0); }
