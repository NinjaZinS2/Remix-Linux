...
// -------------------------------------------------------------- dialogos ---
enum DialogKind { DLG_FOLDER, DLG_IMAGE, DLG_AUDIO_MULTI, DLG_ANYFILE };
...
inline std::string DialogCommand(DialogKind k, const std::string& title) {
    ...
    if (useK) {
        if (k == DLG_ANYFILE) return "kdialog --getopenfilename / --title " + ShQuote(title) + " 2>/dev/null";
    ...
    if (hasZ) {
        if (k == DLG_ANYFILE) return "zenity --file-selection --filename=/usr/bin/ --title=" + ShQuote(title) + " 2>/dev/null";
