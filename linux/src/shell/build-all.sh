...
#   build/remix + src/shell/remix        Linux x86_64 (glibc >= 2.27)
...
# Uso: bash src/shell/build-all.sh   (REMIX_VERSION=1.6.0 bash src/shell/build-all.sh para mudar a versao)
set -euo pipefail
ROOT="$(cd "$(dirname "$0")/../.." && pwd)"
cd "$ROOT"
chmod +x src/shell/*.sh src/shell/packaging/*.sh 2>/dev/null || true
bash src/shell/fetch-deps.sh
bash src/shell/build.sh
bash src/shell/packaging/build-packages.sh
echo
echo "== dist/ =="
